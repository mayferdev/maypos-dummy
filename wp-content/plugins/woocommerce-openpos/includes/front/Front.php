...
//        wp_enqueue_script('openpos.kitchen.nosleep', OPENPOS_URL.'/assets/js/NoSleep.min.js','',$info['Version']);
    ...
    function add_order($is_clear = false){
        ...
        try{
            ...
            if(empty($orders) || $is_clear )
            {
                ...
                // POST REQUEST ---- los datos de: cliente, items, totales (monto, impuestos,....)
                // .....
                //  SUCCESS o FAIL .... .... 
    ...
    public function update_state(){
        ...
        try{
            ...
            $notifications = $op_woo->getNoficications($last_check);
    ...
    public function send_receipt(){
        ...
        try{
            ...
            $order_data = json_decode(stripslashes($_REQUEST['order']),true);
            $send_to = $_REQUEST['to'];
            $register_id = $_REQUEST['cashdrawer_id'];
    ...
    public function get_order_transactions(){
        ...
        try{
            $is_openpos_email = true;
            $session_data = $this->_getSessionData();
            $order_number = stripslashes($_REQUEST['order_number']);
            $transactions = $op_woo->getOrderTransactions($order_number,array( 'order'));
    ...
    public function add_order_transaction(){
        ...
        try{
            ...
            if($in_amount)
            {
                ...
                {
                    $customer_paid_amount += 1  * $t['in_amount'];
                    $customer_paid_amount -= 1 * $t['out_amount'];
                }
                if(($total_paid - $customer_paid_amount) >= $in_amount  )
                {
                    ...
                    throw new Exception(__('Amount not match with remain amount','openpos'));
                }
