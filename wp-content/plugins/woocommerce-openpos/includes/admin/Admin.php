...
//        add_action( 'wp_ajax_openpos_report_data_table', array($this,'report_data_table') );
    ...
    function admin_init() {
        ...
        $allow = false;
        if($current_page =='op-setting' ||  $option_page == 'update'  ||  $option_page == 'openpos')
        {
            $allow = true;
        }
        if( $allow )
    ...
    function get_settings_fields() {
        ...
        if($pos_default_checkout_mode =='single')
        {
            $pos_laybuy = array(
                'name'              => 'pos_laybuy',
    ...
    public function dashboard()
    {
        ...
        foreach($ranges['ranges'] as $r)
        {
            ...
            foreach($transactions as $s)
            {
                ...
                $in = 1* get_post_meta($s->ID,'_in_amount',true);
                $out = 1* get_post_meta($s->ID,'_out_amount',true);
