...
<head>
    ...
    <script>
        ...
        var data_template= '<tr><td class="text-center item-priority" style="background: linear-gradient(90deg, rgba(136,251,63,1) 0% , rgba(252,70,70,1)  <%- priority %>%);"><%= index %></td><td class="item-name"><span class="dining <%- dining %>"><%- dining %></span><%= item %><p class="item-note"><%- note %></p></td><td class="text-center"><%= qty %></td><td><%= order_time %></td><td><%= table %></td><td class="text-center"><% if (done != "ready" && done != "done" ) { %> <a data-id="<%- id %>" href="javascript:void(0);" class="is_cook_ready"> <span class="glyphicon glyphicon-bell" aria-hidden="true"></span> </a> <% } else { %> <span class="glyphicon glyphicon-ok" aria-hidden="true"></span> <% } %></td></tr>';
    </script>
    <?php
    $handes = array(
